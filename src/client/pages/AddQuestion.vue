<template>
	<div v-title :data-title="title" v-loading="loading">
		<van-cell-group>
			<van-field v-model="FormData.titleQue" placeholder="提问问题" autofocus="autofocus" />
		</van-cell-group>
		...
		methods: {
			...
			async Refer() {
				...
				if(this.$route.name == 'EditQuestion') {
					data = await api.patchQuestionDetails(this.FormData)
				} else {
					data = await api.postQuestionDetails(this.FormData)
				}
				await this.$toast({
					message: data.msg,
					duration: 1000
			...
			async getData() {
				if(this.$route.name == 'EditQuestion') {
					this.loading = true
					const data = await api.getQuestionDetails(this.$route.params.id)
					this.FormData.titleQue = data.data.question
					this.FormData.content = data.data.answer
					this.loading = false
				}
			},
		},
